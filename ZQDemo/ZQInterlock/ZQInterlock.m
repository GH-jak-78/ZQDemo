...
- (void)scrollViewPanGestureRecognizer:(UIPanGestureRecognizer *)panGestureRecognizer
{
    if (panGestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        self.contentOffsetY = self.currentScrollView.contentOffset.y < 0.0 ? 0.0 : self.currentScrollView.contentOffset.y;
    }
}
...
- (void)targetViewPanGestureRecognizer:(UIPanGestureRecognizer *)panGestureRecognizer
{
    if (panGestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        [self updataTranslationHeight:self.panTranslationY - panY];
        self.panTranslationY = panY;
    }
}

- (void)updataTranslationHeight:(CGFloat)translationInterval
{
...
//    NSLog(@"%zd  %zd  %f  %f  %f   %zd", self.isTop, self.isBottom, scrollView.contentOffset.y, translationDistance, self.contentOffsetY, scrollView.panGestureRecognizer.state);
    if ((!self.isTop && !self.isBottom && (translationDistance > 0.0 || scrollView.contentOffset.y < 0.0))
        ...
        [self updataTranslationHeight:translationDistance];
...
- (CGFloat)currentTranslationHeight
{
    return -self.targetView.transform.ty;
}
